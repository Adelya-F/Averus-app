                ...
                <div class="flex items-center justify-between p-5 rounded-2xl border transition-all duration-300 {{ $msg->is_read ? 'bg-white border-gray-100 opacity-80' : 'bg-white border-blue-300 shadow-lg shadow-blue-50 ring-1 ring-blue-100' }}">
                    ...
                    <div class="flex items-start gap-4 flex-1">
                        ...
                        <div class="min-w-0 flex-1">
                            <h3 class="font-bold truncate {{ $msg->is_read ? 'text-gray-600' : 'text-gray-800' }}">
                                {{ $msg->title }}
                                @if(!$msg->is_read)
                                    <span class="ml-2 inline-block w-2 h-2 bg-red-500 rounded-full animate-pulse"></span>
                                @endif
                            </h3>
                            <p class="text-sm text-gray-500 mt-1 italic leading-relaxed line-clamp-2">
                                "{{ $msg->message }}"
                            </p>
                    ...
                    <div class="flex-shrink-0 ml-6">
                        <a href="{{ route('admin.inbox.read', $msg->id) }}" class="inline-block bg-gray-800 hover:bg-blue-600 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all transform hover:scale-105 active:scale-95 shadow-sm">
                            CEK DETAIL →
                        </a>
                    </div>
                </div>
